...
class verifyInvDelayb2c(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'InvDelayb2c'
    ...
    def verifyInvDelayRequestInfo(self, req_info, case_id):
        ...
        if 'verifyInvDelayRequestInfo' in all_verify_data:
            ...
            query_dict = self.returnBodyToDicb2c(req_info)
        else:
            raise ValueError(
    ...
    def verifyInvDelayRequestInfob2c(self, case_id, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyInvDelayRequestInfo']
        result = {}
