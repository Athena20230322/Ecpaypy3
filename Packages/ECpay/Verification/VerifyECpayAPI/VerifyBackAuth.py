...
class verifyBackAuth(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'BackAuth'
    ...
    def verifyResponseValuesIssuingBank(self, case_id, res_dict, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_dict = all_verify_data['verifyReturn']
        expect_data = all_verify_data['verifyData']
        ...
        # res_dict['IssuingBank'] = res_dict['IssuingBank'].rstrip(' ')
        # res_dict['IssuingBankCode'] = res_dict['IssuingBankCode'].rstrip(' ')
        if type(expect_dict) is dict:
            if len(expect_dict) > 0:
                result['VerifyResponseValues'] = self.verifyRtnData(res_dict, expect_dict, datatype='dictionary')
            else:
        ...
        print('4524354325')
       # data['IssuingBank'] = 'CTBC Bank'

       # data['IssuingBankCode'] = str(data['IssuingBankCode'])
        data['RtnCode'] = data['RtnCode'].rstrip(' ')
        print('4524354327')
        #data['OrderInfo'] = data['PaymentType'] = 'Credit'
        #data['PaymentType'] = data['PaymentType'].rstrip(' ')
        #data['PaymentType'] = 'Credit'
        data['IssuingBankCode'] = '822'
        data['IssuingBank'] = 'CTBC Bank'
        if type(expect_data) is dict:
            if len(expect_data) > 0:
                result['VerifyDataValues'] = self.verifyRtnData(data, expect_data, datatype='dictionary')
    ...
    def verifyResponseValuesIssuingBankx(self, case_id, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyData']
        print('5533333')
        ...
        # data['OrderInfo'] = str(data['OrderInfo'])

        data['RtnCode'] = data['RtnCode'].rstrip(' ')
        data['CardInfo'] = data['IssuingBank'] = 'CTBC Bank'
        data['CardInfo'] = data['IssuingBankCode'] = '822'
    ...
    def verifyResponseValuesThreeDUnionPay(self, case_id, res_dict, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_dict = all_verify_data['verifyReturn']
        #expect_data = all_verify_data['verifyData']
