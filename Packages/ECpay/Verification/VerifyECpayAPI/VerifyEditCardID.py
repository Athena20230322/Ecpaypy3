...
class verifyEditCardID(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'EditCardID'
    ...
    def verifyReturnResult(self, match_str):
        op = self.webop
        tag_body = op.getElem('body', find_by='tag')
        result = {}
        tag_str = tag_body.text
        try:
            tag_str = tag_str.decode('big5').encode('utf-8')
        except UnicodeDecodeError:
            pass
        finally:
            if tag_str.find(match_str) != -1:
                result['verifyReturnResult'] = True
                return result
            else:
                self.log.WARN('verifyReturnResult: result is not match')
                result['verifyReturnResult'] = False
                return result
    ...
    def verifyEmptyClientRedirectCheck(self, res_text):
        result = {}
        if res_text == '恭喜您，已變更成功！':
            result['verifyEmptyClientRedirectCheck'] = True
            return result
        else:
            result['verifyEmptyClientRedirectCheck'] = False
            raise ValueError("verifyEmptyClientRedirectCheck: Return Message is not success!")