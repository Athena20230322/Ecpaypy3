...
class verifyInvQueryAllowanceInvalidb2b(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'InvQueryAllowanceInvalidb2b'
    ...
    def verifyQueryAllowInvalidResult(self, req_info, case_id):
        ...
        if 'verifyQueryAllowInvalidResult' in all_verify_data:
            ...
            raise ValueError("verifyQueryAllowInvalidResult: Secion 'verifyQueryAllowInvalidResult' not found in Verification.ini")
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                if len(req_info_dict) != 0:
                    result = {}
                    result['verifyQueryAllowInvalidResult'] = self.verifyRtnData(req_info_dict, expect_args_dict, datatype='dictionary')
                    return result
            else:
    ...
    def verifyQueryAllowInvalidResultb2b(self, req_info, case_id):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category,
                                                               self.fea_name,
                                                               case_id,
                                                               'Verification.ini')
        print(all_verify_data['verifyData'])
        print("CCCC")
        if 'verifyData' in all_verify_data:
            expect_args_dict = all_verify_data['verifyData']
        else:
            raise ValueError("verifyData: Secion 'verifyData' not found in Verification.ini")
        print("asdadas5456")
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                if len(req_info_dict) != 0:
                    result = {}
                    result['verifyData'] = self.verifyRtnData(req_info_dict, expect_args_dict, datatype='dictionary')
                    return result
            else:
                raise ValueError("verifyData: Length of expected result dic cannot be zero.")
        else:
            raise TypeError("verifyData: Received expect_args_dict is not an dictionary")
        print("BBBB")
    ...
    def verifyQueryAllowInvalidRequestInfob2b(self, req_info):
            ...
            if len(expect_args_dict) > 0:
                if len(expect_args_dict) != 0:
                    if expect_args_dict['RtnCode'] is '1' :
                        result = {}
                        result['verifyInvQueryAllowanceRequestInfo'] = self.verifyRtnData(query_dict, expect_args_dict,
                                                                                          datatype='dictionary')
                        return result
                    else:
                        Error_result = 'Result: ' + req_info
                        self.log.WARN(Error_result)
                        raise ValueError(
                            "verifyQueryAllowInvalidRequestInfo: RtnCode is not one or ReCheckMacValue is not match.")
            else:
                raise ValueError("verifyQueryAllowInvalidRequestInfo: Length of expected result dic cannot be zero.")
        else:
            raise TypeError("verifyQueryAllowInvalidRequestInfo: Received expect_args_dict is not an dictionary.")