...
class verifyInvAllowanceByCollegiate(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'InvAllowanceByCollegiate'
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                ...
                if mode == 'success':
                    ...
                    if all_rtn['IA_Allow_No'] is not False and all_rtn['IA_TempDate'] is not False:
                        result['verifyAllowNoAndDate'] = True
                    else:
                        result['verifyAllowNoAndDate'] = False
                        self.log.WARN("verifyInvAllowanceReturn: IA_Allow_No or IA_TempDate is empty!\n"
                                      "IA_Allow_No=%s\nIA_TempDate=%s" % (all_rtn['IA_Allow_No'], all_rtn['IA_TempDate']))
                    ...
                    if all_rtn['IA_Allow_No'] and all_rtn['IA_Invoice_No'] and all_rtn['IA_TempDate'] is False:
                        result['verifyFailCase'] = True
                    else:
                        result['verifyFailCase'] = False
                        self.log.WARN("verifyInvAllowanceReturn: IA_Allow_No or IA_Invoice_No or IA_TempDate is not empty\n"
                                      "IA_Allow_No=%s.\nIA_Invoice_No=%s\nIA_TempDate=%s" % (all_rtn['IA_Allow_No'],
                                                                                         all_rtn['IA_Invoice_No'],
                                                                                         all_rtn['IA_TempDate']))
