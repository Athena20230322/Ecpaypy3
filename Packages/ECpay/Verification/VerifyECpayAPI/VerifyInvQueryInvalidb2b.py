...
class verifyInvQueryInvalidb2b(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'InvQueryInvalidb2b'
    ...
    def verifyResponseValueb2b(self, res_dict, issue_dict, issue_rtn, invalid_dict, case_id):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_args_dict = all_verify_data['verifyReturnValue']
        print(expect_args_dict)

        issue_dict = self.returnBodyToDicb2b(issue_dict)
        res_dict = self.returnBodyToDicb2b2(res_dict)
        print(res_dict)

        issue_rtn = self.returnBodyToDicb2b(issue_rtn)
        invalid_dict =self.returnBodyToDicb2b(invalid_dict)
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                ...
                if res_dict['MerchantID'] == issue_dict['MerchantID']:


                    result['verifyIIS_Mer_ID'] = True
                else:
                    self.log.WARN("IIS_Mer_ID not match!\nInvQueryInvalid: %s\nInvIssue: %s" % (res_dict['IIS_Mer_ID'], issue_dict['MerchantID']))
                    result['verifyIIS_Mer_ID'] = False
                if res_dict['InvoiceNumber'] == issue_rtn['InvoiceNumber']:
                ...
                if res_dict['ConfirmDate'] != '':
                    result['verifyConfirmDate'] = True
                else:
                    self.log.WARN("ConfirmDate is empty!")
                    result['verifyConfirmDate'] = False
                if res_dict['Upload_Date'] == 'null':
                    result['verifyUpload_Date'] = True
                else:
                    result['verifyUpload_Date'] = False
                print(invalid_dict)
                print(res_dict)
                if res_dict['CancelReason'] == invalid_dict['Reason']:
                    result['verifyCancelReason'] = True
                else:
                ...
                if res_dict['SellerId'] == '53538851':
                    result['verifySeller_Identifier'] = True
                else:
                    self.log.WARN("Seller_Identifier not match!\nInvQueryInvalid: %s" % res_dict['Seller_Identifier'])
                    result['verifySeller_Identifier'] = False
                if res_dict['BuyerId'] == issue_dict['CustomerIdentifier']:
                    result['verifyBuyer_Identifier'] = True
                elif res_dict['BuyerId'] == '22422980':
                    result['verifyBuyer_Identifier'] = True
                else:
