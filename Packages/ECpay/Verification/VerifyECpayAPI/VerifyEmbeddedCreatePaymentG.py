...
class verifyEmbeddedcreatepaymentG(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'EmbeddedCreatePaymentG'
    ...
    def verifyResponseValuesThreeD(self, case_id, res_dict, data, status='success'):
        ...
        if status == 'success':
            ...
            result['verifyMustEmptyValues'] = not self.__valueIsEmpty(is_empty_check_dict)
        return result
    def verifyResponseValuesforcreatepayment(self, case_id, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyData']
        print('5533333')
    ...
    def verifyResponseValuesforcreatepaymentGM(self, case_id, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyData']
        print('5533333')
        ...
        data['RtnCode'] = str(data['RtnCode'])
        data['RtnCode'] = data['RtnCode'].rstrip(' ')
        data['CoBrandingInfo'] = data['CoBrandingCode'] = 'autotest'
        data['CoBrandingInfo'] = data['Comment'] = 'autotest1234'
