...
class verifyInvQueryInvalidb2c(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'InvQueryInvalidb2c'
    ...
    def verifyResponseValueb2c(self, res_dict, issue_dict, issue_rtn, invalid_dict, case_id):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_args_dict = all_verify_data['verifyReturnValue']
        print(expect_args_dict)
        issue_dict = self.returnBodyToDicb2c(issue_dict)
        res_dict = self.returnBodyToDicb2c(res_dict)
        issue_rtn = self.returnBodyToDicb2c(issue_rtn)
        invalid_dict =self.returnBodyToDicb2c(invalid_dict)
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                ...
                if res_dict['IIS_Mer_ID'] == issue_dict['MerchantID']:
                ...
                if res_dict['II_Invoice_No'] == issue_rtn['InvoiceNo']:
                    result['verifyII_Invoice_No'] = True
                else:
                    self.log.WARN("II_Invoice_No not match!\nInvQueryInvalid: %s\nInvIssueRtn: %s" % (res_dict['II_Invoice_No'], issue_rtn['InvoiceNo']))
                ...
                print(invalid_dict)
                print(res_dict)
                if res_dict['Reason'] == invalid_dict['Reason']:
