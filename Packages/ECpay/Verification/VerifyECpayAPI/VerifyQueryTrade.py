...
class verifyQueryTrade(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'QueryTrade'
    ...
    def verifyResponseValuesQueryThreeDBackCode(self, case_id, res_dict, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_dict = all_verify_data['verifyReturn']
        expect_data = all_verify_data['verifyData']
       # expect_ChargeFee = all_verify_data['verifyChargeFee']
        result = {}
        res_dict['TransMsg'] = str(res_dict['TransMsg'])
        res_dict['MerchantID'] = str(res_dict['MerchantID'])
        res_dict['TransCode'] = str(res_dict['TransCode'])
       ...
       # ExecLog = data['ExecLog']
        #ExecLogArray = ExecLog[0]
       # ExecLogArray['ChargeFee'] = str(ExecLogArray['ChargeFee'])
        # data['AuthCode'] = str(data['AuthCode'])
        # data['amount'] = str(data['amount'])
        data['RtnCode'] = data['RtnCode'].rstrip(' ')
        if type(expect_data) is dict:
    ...
    def verifyResponseValuesQueryThreeDQ(self, case_id, res_dict, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_dict = all_verify_data['verifyReturn']
        expect_data = all_verify_data['verifyData']
        ...
        if type(expect_ChargeFee) is dict:
            if len(expect_ChargeFee) > 0:
                result['VerifyDataValues'] = self.verifyRtnData(ExecLogArray, expect_ChargeFee, datatype='dictionary')
            else:
                raise ValueError("verifyResponseValues: Length of expect_data cannot be zero.")
        else:
            raise TypeError("verifyResponseValues: Received expect_data is not a dictionary.")
