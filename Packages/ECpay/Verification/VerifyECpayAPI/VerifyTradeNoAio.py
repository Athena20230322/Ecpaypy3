...
class verifyTradeNoAio(APIHelper):
    ...
    def verifyStatusResult(self, res_info, match_str):
        result = {}
        try:
            res_info = res_info.decode('big5').encode('utf-8')
    ...
    def verifyCSVReturnDownload(self, query_order_dict, case_id):
        ...
        if 'verifyCSVReturn' in all_verify_data:
            ...
            raise ValueError("verifyCSVReturnDownload: Secion 'verifyCSVReturn' not found in Verification.ini")
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                if len(query_order_dict) != 0:
                    result = {}
                    result['verifyCSVReturnDownload'] = self.verifyRtnData(query_order_dict, expect_args_dict,
                                                                           datatype='dictionary')
                    return result
            else:
                raise ValueError("verifyCSVReturnDownload: Length of expected result dic cannot be zero.")
        else:
            raise TypeError("verifyCSVReturnDownload: Received expect_args_dict is not an dictionary")
    ...
    def verifyStrCSVReturn(self, query_order, case_id):
        ...
        try:
            coworker_remark_enter = '\xb2\xce\xa4@\xbds\xb8\xb9\r\n'
            coworker_remark = "\xb2\xce\xa4@\xbds\xb8\xb9"
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                if len(query_order_dict) != 0:
                    result = {}
                    result['verifyStrCSVReturn'] = self.verifyRtnData(query_order_dict, expect_args_dict,
                                                                      datatype='dictionary')
                    return result
            else:
