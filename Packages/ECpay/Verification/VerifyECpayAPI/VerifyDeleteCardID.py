...
class verifyDeleteCardID(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'DeleteCardID'
    ...
    def verifyResponseValues(self, res_dict, case_id):
        result = {}
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        if 'verifyValues' in all_verify_data:
            expect_args_dict = all_verify_data['verifyValues']
        else:
            raise ValueError(
                "verifyValues: Secion 'verifyValues' not found in Verification.ini")
