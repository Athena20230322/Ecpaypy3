...
class verifyEmbeddedUI(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'EmbeddedUI'
    ...
    def verifyResponseValuesb2cx(self, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyData']
        result = {}
        # for key in data.keys():
        ...
        #         data[key] = ''
        data['RtnCode'] = str(data['RtnCode'])
        data['LogisticsID'] = str(data['LogisticsID'])
        #data['LogisticsSubType'] = str(data['LogisticsSubType'])
    ...
    def verifyResponseValuesEmbeddedUI(self, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyData']
        result = {}
    ...
    def verifyClientRedirectColumnsCheck(self, res_dict, case_id):
        ...
        if res_dict['TempLogisticsID'] != '':
            insert_value_list = ['TempLogisticsID']
        else:
            insert_value_list = []
        print(res_dict)
        print((res_dict['RtnCode']))
        res_dict['RtnCode'] = str(res_dict['RtnCode'])
        print((res_dict['RtnCode']))
    ...
    def verifyClientRedirectColumnsCheckL(self, res_dict, case_id):
        result = {}
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        if 'verifyClientRedirectColumns' in all_verify_data:
    ...
    def verifyCreateTestDataCheck(self, req_info):
        expect_args_dict = self.returnBodyToDic(req_info)
        tamp_dic = {}
        tamp_dic['TempLogisticsID'] = expect_args_dict['1|TempLogisticsID']
        expect_args_dict.pop('1|TempLogisticsID')
        expect_args_dict['TempLogisticsID'] = tamp_dic['TempLogisticsID']
        query_dict = {}
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                if len(query_dict) != 0:
                    if query_dict['RtnCode'] == '300' or query_dict['RtnCode'] == '2001':

                        result['verifyCreateTestDataCheck'] = self.verifyRtnData(query_dict,
                                                                                 expect_args_dict,
                                                                                 datatype='dictionary')
                        return result
                    else:
                        self.log.WARN("RtnCode is not '300 or 2001'!%s" % query_dict)
                        result['verifyCreateTestDataCheck'] = False
                        return result

            else:
                raise ValueError("verifyCreateTestDataCheck: Length of expected result dic cannot be zero.")
        else:
            raise TypeError("verifyCreateTestDataCheck: Received expect_args_dict is not an dictionary.")
