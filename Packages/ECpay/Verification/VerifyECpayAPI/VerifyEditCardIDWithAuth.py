...
class verifyEditCardIDWithAuth(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'EditCardIDWithAuth'
    ...
    def verifyClientRedirectColumnsCheck(self, res_dict, case_id, mert_no):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_args_dict = all_verify_data['verifyClientRedirectColumns']
        insert_value_list = ['BindingDate', 'AllpayTradeNo', 'gwsr', 'process_date', 'CardID','MerchantMemberID','CheckMacValue']
        expect_args_dict['MerchantTradeNo'] = mert_no
        for key in insert_value_list:
                expect_args_dict[key] = res_dict[key]
        #expect_args_dict.pop('CheckMacValue')
