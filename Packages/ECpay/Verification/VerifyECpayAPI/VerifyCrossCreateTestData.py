...
class verifyCrossCreateTestData(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'CrossCreateTestData'
    ...
    def verifyCreateHiLifeTestDataResult(self):
        ...
        if tag_str[0].find('1') != -1:
            result['verifyCreateTestDataResult'] = True
            return result
        else:
            self.log.WARN('verifyCreateTestDataResult: result is not 1|return_message!')
            result['verifyCreateTestDataResult'] = False
            return result
    ...
    def verifyCreateHiLifeTestDataDownload(self, merc_id):
        ...
        if tag_str[0].find('1') != -1:
            ...
            if tag_str.find(merc_id) != -1:
                result['verifyCreateTestDataDownload'] = True
                return result
        else:
            self.log.WARN('verifyCreateTestDataResult: result is not 1|return_message!')
            result['verifyCreateTestDataDownload'] = False
            return result
