...
class verifyInvQueryb2b(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'InvQueryb2b'
    ...
    def verifyColumn(self,  case_id,res_dict):
        ...
        res_dict=self.returnBodyToDicb2b(res_dict)
    ...
    def verifyResponseValue(self, res_dict, inv_dict, inv_rtn, case_id):
        ...
        print(expect_args_dict)
        inv_dict = self.returnBodyToDicb2b(inv_dict)
        res_dict = self.returnBodyToDicb2b(res_dict)
        inv_rtn = self.returnBodyToDicb2b(inv_rtn)
        print('456')
        print(inv_dict)
        print('123')
        result = {}
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
               # revert_list = ['PosBarCode', 'QRCode_Left', 'QRCode_Right']
                revert_dict = {}
               ...
               # res_dict.pop('QRCode_Right')
                ...
                res_dict.pop('Buyer_Address')
                res_dict.pop('RtnData')
                res_dict.pop('Buyer_TelephoneNumber')
                res_dict.pop('ExchangeStatus')
                res_dict.pop('Buyer_EmailAddress')
                res_dict.pop('InvoiceDate')
                res_dict.pop('Buyer_FacsimileNumber')
                res_dict.pop('Buyer_Name')
                res_dict.pop('CreateDate')
                res_dict.pop('ConfirmDate')
    ...
    def verifyResponseValueb2b(self, case_id, data, status='success'):
            all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                                   'Verification.ini')
    ...
    def compare(self, key, res_dict, inv_dict, inv_rtn):
        correspond_dict = {
            ...
            'InvoiceNumber': ['inv_rtn', 'InvoiceNumber'],
            'RelateNumber': ['inv_dict', 'RelateNumber'],
            'IIS_Customer_ID': ['inv_dict', 'CustomerID'],
            'Buyer_Identifier': ['inv_dict', 'CustomerIdentifier'],
            ...
            'CustomsClearanceMark': ['inv_dict', 'ClearanceMark'],
            'InvoiceType': ['inv_dict', 'InvType'],
            'IIS_Category': 'B2C',
            'TaxType': ['inv_dict', 'TaxType'],
            'TaxRate': '0.05',
            'IIS_Tax_Amount': ['inv_dict', 'ItemCount', 'ItemPrice', 'SalesAmount'],
            'TotalAmount': ['inv_dict', 'TotalAmount'],
            'ItemTax': ['inv_dict', 'ItemTax'],
            'SalesAmount': ['inv_dict', 'SalesAmount'],
            'IIS_Check_Number': 'P',
            ...
            'RandomNumber': ['inv_rtn', 'RandomNumber'],
            'MainRemark': ['inv_dict', 'InvoiceRemark'],
            'TaxAmount': ['inv_dict', 'TaxAmount'],

        }
        if key == 'Buyer_Identifier':
            ...
            if res_dict['Buyer_Identifier'] != '0000000000':
                self.log.INFO('res_dict[Buyer_Identifier]=%s' % (res_dict['Buyer_Identifier']))
                self.log.INFO('res_dict[%s]=%s' % (key, res_dict[key]))
            ...
            if res_dict['IIS_Carruer_Type'] == '1':
                ...
                self.log.INFO('res_dict[%s]=%s' % (key, res_dict[key]))
                return True if res_dict[key] == '53538851' + res_dict['RelateNumber'] else False
        else:
