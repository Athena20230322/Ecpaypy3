...
class verifyEmbeddedcreatepaymentweb(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'ECpayAPI'
        self.fea_name = 'EmbeddedCreatePaymentWeb'
    ...
    def verifyResponseValuesforcreatepaymentOrd(self, case_id, data, status='success'):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        expect_data = all_verify_data['verifyData']
        print('5533333')
