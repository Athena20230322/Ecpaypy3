...
class actQueryTradeCoBranding(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='QueryTradeCoBranding')
    ...
    def genPostRequestToAPIN(self, req_info):
        form_act = self.feature_conf['QueryTradeN_API']
        od_arg_dict = collections.OrderedDict(sorted(list(req_info.items()), key=lambda x: x[0].lower()))
        # od_arg_dict = json.dumps(od_arg_dict)
        response = self.postRequestToAPIThreeD(self.genSession(), od_arg_dict, form_act, trencode_dotnet=True)
        self.log.INFO(response.decode('utf-8'))
