...
class actCreditCardPeriodAction(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='CreditCardPeriodAction')
    ...
    def genOrderRequestInfoPID(self, param_csv,chkmac='local'):
        req = self.genArgsDictFromCSV(param_csv)
        chksum = self.genChkMacVal(req, mode=chkmac, codec='sha256', case='insensitive1')  # 驗證
        print(chksum)
        req['CheckMacValue'] = chksum  # 檢查碼
        print(req['MerchantTradeNo'])  # 廠商交易編號
        return req
    def genOrderRequestInfo1(self, param_csv,MerchantTradeNo,chkmac='local'):
        req = self.genArgsDictFromCSV(param_csv)
    ...
    def genOrderRequestInfo2(self, param_csv,MerchantTradeNo,chkmac='local'):
        req = self.genArgsDictFromCSV(param_csv)
        if req['MerchantTradeNo']=='':
            req['MerchantTradeNo']=MerchantTradeNo
        chksum = self.genChkMacVal(req, mode=chkmac, codec='sha256', case='insensitive1')  # 驗證
    ...
    def genOrderRequestInfoPID2(self, param_csv, chkmac='local'):
        req = self.genArgsDictFromCSV(param_csv)
    ...
    def genPostRequestToAPI(self, req_info):
        form_act = self.feature_conf['CreditCardPeriodAction_API']
    ...
    def inputPaymentPageCreditInfoE1(self, card_no, exp_year, exp_month, cvv, cell_no, email, name, mode, rebind=False):
        op = self.webop
        cno_parts = card_no.split('-')
        op.handleAlert()
        if mode != 'divide':
