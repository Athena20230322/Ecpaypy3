...
class actBackAuthAN(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='BackAuthAN')
    ...
    def genOrderRequestInfoThreeD2UnionPay(self, param_csv):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        CVSInfo = self.genArgsDictFromCSV(param_csv)
        OrderInfo = self.genArgsDictFromCSV(param_csv)
        ...
        UnionPayInfo.pop('OrderInfo')
        UnionPayInfo.pop('ChoosePayment')
        UnionPayInfo.pop('ConsumerInfo')
    ...
    def genOrderRequestInfoThreeD2ApplePay(self, param_csv):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
       # CVSInfo = self.genArgsDictFromCSV(param_csv)
        OrderInfo = self.genArgsDictFromCSV(param_csv)
        ...
        ConsumerInfo = self.genArgsDictFromCSV(param_csv)
        ApplePayInfo = self.genArgsDictFromCSV(param_csv)
        req = {}
        req['MerchantID'] = data['MerchantID']
        # req['PlatformID'] = '3083192'
        data.pop('Timestamp')
       ...
       # data.pop('ApplePayInfo')
        ...
        rqHeader.pop('ReturnURL')
        rqHeader.pop('ChoosePayment')
        rqHeader.pop('ApplePayInfo')
        rqHeader.pop('ConsumerInfo')
        rqHeader.pop('CurrencyCode')
        rqHeader.pop('TradeType')
        #rqHeader.pop('PaymentToken')
        rqHeader.pop('Email')
        rqHeader.pop('Phone')
        rqHeader.pop('Name')
        rqHeader.pop('CountryCode')
        ...
        ConsumerInfo.pop('ReturnURL')
        ConsumerInfo.pop('ChoosePayment')
        ConsumerInfo.pop('ApplePayInfo')
        ConsumerInfo.pop('CurrencyCode')
        ConsumerInfo.pop('TradeType')
        ...
        OrderInfo.pop('PlatformID')
        OrderInfo.pop('ChoosePayment')
        OrderInfo.pop('CurrencyCode')
        OrderInfo.pop('TradeType')
        OrderInfo.pop('ApplePayInfo')
        OrderInfo.pop('PaymentToken')
        OrderInfo.pop('ConsumerInfo')
        OrderInfo.pop('Email')
        OrderInfo.pop('Phone')
        OrderInfo.pop('Name')
        ...
        ApplePayInfo.pop('Timestamp')
        ApplePayInfo.pop('RqID')
        ApplePayInfo.pop('Revision')
        ApplePayInfo.pop('MerchantID')
        ApplePayInfo.pop('PlatformID')
        ApplePayInfo.pop('OrderInfo')
        ApplePayInfo.pop('ChoosePayment')
        ApplePayInfo.pop('CurrencyCode')
        ApplePayInfo.pop('TradeType')
        ApplePayInfo.pop('PaymentToken')
        ApplePayInfo.pop('ConsumerInfo')
        ApplePayInfo.pop('Email')
        ApplePayInfo.pop('Phone')
        ApplePayInfo.pop('Name')
        ApplePayInfo.pop('CountryCode')
        ApplePayInfo.pop('Address')
        ApplePayInfo.pop('CustomField')

        data['OrderInfo'] = OrderInfo
       # data['CVSInfo'] = CVSInfo
        data['ConsumerInfo'] = ConsumerInfo
        data['ApplePayInfo'] = ApplePayInfo
    ...
    def strToDictNo3DUnionPay(self, res_info):
        res_info = self.urlEncode(res_info)
        print('str1: ' + res_info)
        res_dict = {}
        res_info = self.urlDecode(res_info)
    ...
    def createShopifyByBrowser(self, unionpayurl):
        op = self.webop
        op.goToURL(unionpayurl)
        time.sleep(5)
    ...
    def decryptDataNo3DUnionPay(self, data):
        # data = self.urlDecode(data)
