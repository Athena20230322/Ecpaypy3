...
class actCreditCardPeriodActionB(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='CreditCardPeriodActionB')
    ...
    def genOrderRequestInfo(self, param_csv,MerchantTradeNo):
    ...
    def genOrderRequestInfoPID(self, param_csv, MerchantTradeNo):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        req = {}
        if data['MerchantTradeNo'] == '':
            data['MerchantTradeNo'] = MerchantTradeNo
        req['MerchantID'] = data['PlatformID']
        data.pop('Timestamp')
        data.pop('RqID')
        data.pop('Revision')
        rqHeader.pop('MerchantID')
        ...
        data = self.urlEncode(data)
        print(data)
        data = self.aesEncryptPID(data)
        print(data)
        req['Data'] = data
        req['RqHeader'] = rqHeader
        return req
    def genOrderRequestInfo1(self, param_csv):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        req = {}
        req['MerchantID'] = data['MerchantID']
    ...
    def createOrderByRequestThreeD(self, req_info):
        form_act = self.feature_conf['CreditCardPeriodActionB_API']
        od_arg_dict = collections.OrderedDict(sorted(list(req_info.items()), key=lambda x: x[0].lower()))
    ...
    def decryptData3D(self, data):
        ...
        print(data)
        return json.loads(self.aesDecrypt(data))
    def decryptData3DPID(self, data):
        #data = self.urlDecode(data)
        print(data)
        return json.loads(self.aesDecryptPID(data))
