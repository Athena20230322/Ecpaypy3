...
class actEmbeddedtokenv4(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='EmbeddedToken')
    ...
    def genOrderRequestInfoGetTokenBindingCardv4(self, param_csv, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
    ...
    def genOrderRequestInfoGetTokenBindingCard(self, param_csv, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
    ...
    def genOrderRequestInfoGetTokenbyBindingCardv4(self, param_csv, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
       ...
       # data.pop('MerchantTradeDate')
        ...
        rqHeader.pop('CustomField')
        rqHeader.pop('BindCardResultURL')
        rqHeader.pop('BindCardReturnURL')
        rqHeader.pop('MerchantTradeNo')
        rqHeader.pop('TotalAmount')
        ...
        ConsumerInfo.pop('PlatformID')
        ConsumerInfo.pop('CustomField')
        ConsumerInfo.pop('BindCardResultURL')
        ConsumerInfo.pop('BindCardReturnURL')
        ConsumerInfo.pop('MerchantTradeNo')
        ConsumerInfo.pop('TotalAmount')
    ...
    def genOrderRequestInfoThreeD2v4(self, param_csv, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
    ...
    def genOrderRequestInfoThreeD2GMIDv4(self, param_csv, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        OrderInfo = self.genArgsDictFromCSV(param_csv)
        ConsumerInfo = self.genArgsDictFromCSV(param_csv)
    ...
    def genPostRequestToAPIv4(self, req_info):
        form_act = self.feature_conf['Appsdktokenv4_API']
        od_arg_dict = collections.OrderedDict(sorted(list(req_info.items()), key=lambda x: x[0].lower()))
        response = self.postRequestToAPIb2c(self.genSession(), od_arg_dict, form_act, trencode_dotnet=True)
        self.log.INFO(response.decode('utf-8'))
        return response
