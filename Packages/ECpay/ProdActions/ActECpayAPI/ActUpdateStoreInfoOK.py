...
class actUpdateStoreInfoOK(APIHelper):
    ...
    def createOrderByRequest(self, req_info):
        ...
        self.log.INFO("createOrderByRequest: " + response.text)
    ...
    def getParamsFromResponse(self, response):
        ...
        self.log.INFO('getParamsFromResponse: AllPayLogisticsId' + params['logisticsID'])
        self.log.INFO('getParamsFromResponse: CVSPaymentNo' + params['CVSPaymentNo'])
        self.log.INFO('getParamsFromResponse: CVSValidationNo' + params['CVSValidationNo'])
        return params

    def genUpdateStoreInfo(self, param_csv, allpay_logistics_id, cvs_payment_no, cvs_validation_no):
        req = self.genArgsDictFromCSV(param_csv, de_strip=True)
    ...
    def updateStoreInfoByRequest(self, req_info):
        form_act = self.feature_conf['Update_StoreOK_API']
        response = requests.post(form_act, data=req_info)
        self.log.INFO("updateStoreInfoByRequest: " + response.text)
        return response.text
