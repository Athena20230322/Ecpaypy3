...
class actEmbeddedtokenGv4(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='EmbeddedTokenGv4')
    ...
    def genOrderRequestInfoThreeD2Flex(self, param_csv, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
    ...
    def createECPAYByBrowser(self):
        op = self.webop
        op.goToURL('https://vendor-stage.ecpay.com.tw/User/LogOn_Step1_automation')

        time.sleep(5)
        return

    def Login(self, account,IDN,pw):
        op = self.webop
        time.sleep(5)
        op.inputTextBox('Account', account)
        ...
        op.clickElem('LoginAllPay')
        time.sleep(5)
        op.clickElem('/html/body/section/div/div/div[2]/ul/li[3]/a/text()', findby='xpath')
