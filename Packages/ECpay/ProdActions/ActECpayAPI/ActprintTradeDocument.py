...
class actprintTradeDocument(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='printTradeDocument')
    ...
    def genOrderRequestInfo(self, param_csv, alplogis_id, inject_value=''):
        req = self.genArgsDictFromCSV(param_csv)
    ...
    def createOrderByBrowser(self, browser_drv, req_info, api_url):
        op = self.webop
        form_actprintTradeDocument = self.feature_conf[api_url]
        form_genorder = self.createHtmlFormJs('FormGenOrder', req_info, action=form_actprintTradeDocument, submit=True)
        print(form_genorder)
    ...
    def genPostRequestToAPI(self, req_info, api_url='Create_API'):
    ...
    def modifyAllpaylogisticsSplit(self, modify_str):
        ...
        if type(modify_str) is str:
            ...
            raise TypeError("modifyAllpaylogisticsSplit: Paramter type is not a str")
