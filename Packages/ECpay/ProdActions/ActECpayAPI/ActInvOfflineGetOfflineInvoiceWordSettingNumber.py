...
class actInvOfflineGetOfflineInvoiceWordSettingNumber(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='InvOfflineGetOfflineInvoiceWordSettingNumber')
    ...
    def genOrderRequestInfoInvOfflineGetOfflineInvoiceWordSetting(self, param_csv ):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
    ...
    def genOrderRequestInfoInvOfflineGetOfflineInvoiceWordSettingNumber(self, param_csv):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
    ...
    def genPostRequestToAPIWordSettingNumber(self, req_info):
        form_act = self.feature_conf['InvQueryb2cWordSettingNumber_API']
        od_arg_dict = collections.OrderedDict(sorted(list(req_info.items()), key=lambda x: x[0].lower()))
        response = self.postRequestToAPIb2c(self.genSession(), od_arg_dict, form_act, trencode_dotnet=True)
        self.log.INFO(response.decode('utf-8'))
        return response
    ...
    def strToDictNumber(self, res_info):
        res_info = self.urlEncode(res_info)
        print('str1: ' + res_info)
        res_dict = {}
        res_info = self.urlDecode(res_info)
