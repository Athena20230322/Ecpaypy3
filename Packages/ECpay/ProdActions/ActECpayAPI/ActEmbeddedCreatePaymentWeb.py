...
class actEmbeddedcreatepaymentweb(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='EmbeddedCreatePaymentWeb')
    ...
    def genOrderRequestInfoCreatePaymentWebv4(self, param_csv, create, order):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        order = json.loads(order)
        create = json.loads(create)
        merchanttrade = order['OrderInfo']
        req = {}
        ...
        rqHeader.pop('MerchantID')
        rqHeader.pop('PlatformID')
        rqHeader.pop('BindCardPayToken')
        rqHeader.pop('MerchantMemberID')

        data['BindCardPayToken'] = create['BindCardPayToken']
        data['MerchantMemberID'] = merchanttrade['MerchantMemberID']
    ...
    def genOrderRequestInfoCreatePaymentWeb(self, param_csv, create, order):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        order = json.loads(order)
        create = json.loads(create)
        ...
        rqHeader.pop('MerchantID')
        rqHeader.pop('PlatformID')
        rqHeader.pop('PayToken')
        rqHeader.pop('MerchantTradeNo')

        data['PayToken'] = create['PayToken']
        data['MerchantTradeNo'] = merchanttrade['MerchantTradeNo']
    ...
    def genOrderRequestInfoCreatePaymentWebold(self,param_csv, create,order):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        order = json.loads(order)
        create=json.loads(create)
    ...
    def genOrderRequestInfoCreatePayment(self, param_csv, paytoken, query_rtn):
        print('111111')
        data={}
        paytoken=json.loads(paytoken)
        data['PayToken']=paytoken['PayToken']
        headers = query_rtn.headers
    ...
    def genPostRequestToAPIPayment(self, req_info, keyinfo, data2):
        rqq = {}
        form_act = self.feature_conf['AppsdkPayment_API']
        print(req_info)
        req_info1 = {}
        req_info1['Data'] = req_info
        req_info1 = json.dumps(req_info1)
        ...
        req = json.dumps(req_info)
        print('213123124')
        response = self.postRequestToAPIQueryL(self.genSession(), req_info1, form_act, Language, UUID, AppName, AppInfo,
                                               KeyInfo,
                                               Signature, KeyID,
                                               trencode_dotnet=True)
        print('2131231256')
    ...
    def genPostRequestToAPIv4(self, req_info):
        form_act = self.feature_conf['AppsdkPaymentv4_API']
        od_arg_dict = collections.OrderedDict(sorted(list(req_info.items()), key=lambda x: x[0].lower()))
        response = self.postRequestToAPIb2c(self.genSession(), od_arg_dict, form_act, trencode_dotnet=True)
        self.log.INFO(response.decode('utf-8'))
        return response
    ...
    def encryptDatapppsdkn(self, data, skey):
        ...
        data = data.headers
        print(data['KeyInfo'])
        data = self.rsaDecrpt(data['KeyInfo'])
        print(data)
        print('888877777776666')
