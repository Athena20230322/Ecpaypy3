...
class actprintTradeDocumentV2(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='ECpayAPI', feature_name='printTradeDocumentV2')
    ...
    def genOrderRequestInfoB2cPr(self, param_csv):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        # Items = self.genArgsDictFromCSV(param_csv)
        req = {}
    ...
    def genOrderRequestInfoB2cLog(self, param_csv, create, inject_value=''):
        ...
        print(req)
        return req
    def genOrderRequestInfoC2cLog(self, param_csv, create, inject_value=''):
        data = self.genArgsDictFromCSV(param_csv)
        rqHeader = self.genArgsDictFromCSV(param_csv)
        ls = self.genArgsDictFromCSV(param_csv)
        req = {}
    ...
    def strToDictP(self, res_info):
        ...
        print(res_dict)
        return res_dict
    def genPostRequestToAPI(self, req_info,save_path):
        op = self.webop
        form_act = self.feature_conf['PrintV2_API']
        ...
        html_tmp = os.path.join(save_path, 'print.html')
    ...
    def cleanHtmlTmp(self, html_path):
        time.sleep(5)
        file = os.path.join(html_path, 'print.html')
        time.sleep(5)
        os.remove(file)
    ...
    def getPrintElementsHAI(self):
        op = self.webop
        time.sleep(20)
        eles = {}
        op.clickElem('/html/body/embed', findby='xpath')
        # test = op.getElem('/html/body/embed',
        #                              find_by='xpath').click
        # print test
