...
class actConsoleLogin(webOperate):
    def __init__(self, drv, log_file):
        ...
        self.feature_conf = self.helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='AllpayProdSemiAuto', feature_name='ConsoleLogin')
                ...
                if act.__contains__('/DumpAuth/AuthAbandon'):
                    ...
                    for sub in subelems:
                        ...
                        if sub.get_attribute('type') == 'submit' and elem_val == btn_value:
                            ...
                            time.sleep(1)
                            self.handleAlert(action='Accept')
                            time.sleep(1)
                            self.handleAlert(msg='ok', action='Accept')
                            time.sleep(1)
        else:
