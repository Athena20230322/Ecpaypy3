...
class actprintTradeDocument(APIHelper):
    def __init__(self):
        ...
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='AllpayAPI', feature_name='printTradeDocument')
    ...
    def genOrderRequestInfo(self, param_csv, alplogis_id):
        req = self.genArgsDictFromCSV(param_csv)
        if alplogis_id is None:
            req['AllPayLogisticsID'] = ''
            print(ValueError("genOrderRequestInfo: input AllPayLogisticsID is None"))
        else:
