...
class actEditCardIDRedirect(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='AllpayAPI', feature_name='EditCardIDRedirect')
    ...
    def genOrderRequestInfo(self, param_csv, binding_info):
    ...
    def genPostRequestToAPI(self, req_info, api_url):
        form_act = self.feature_conf[api_url]
    ...
    def GetInfoFromServerReplyUrl(self, merchant_trade_no):
        serverReplyUrl = 'http://192.168.50.88:5000/?IdentCol=MerchantTradeNo&IdentVal=' + merchant_trade_no
    ...
    def getCloseMessage(self):
        op = self.webop
        divbody = op.getElem('info', find_by='class').text
        self.log.INFO(divbody)
        return divbody.encode('utf-8')
    ...
    def GetInfoFromPageWithoutClientURL(self):
        op = self.webop
        divText = op.drv.find_element_by_tag_name('h3').text
        self.log.INFO(divText)
        return divText
