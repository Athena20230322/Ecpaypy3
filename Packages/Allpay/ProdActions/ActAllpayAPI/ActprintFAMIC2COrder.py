...
class actprintFAMIC2COrder(APIHelper):
    def __init__(self):
        self.t_helper = classTestHelper()
        self.feature_conf = self.t_helper.getFeatureTestSetting('ModuleWideInfo', feature_cat='AllpayAPI', feature_name='printFAMIC2COrder')
    ...
    def genOrderRequestC2C(self, param_csv, list_index):
        req = self.genArgsDictFromCSV(param_csv)
        if list_index is None:
            req['AllPayLogisticsID'] = ''
            req['CVSPaymentNo'] = ''
            print(ValueError("genOrderRequestC2C: input AllPayLogisticsID & CVSPaymentNo is None"))
        else:
    ...
    def createOrderByBrowser(self, browser_drv, req_info, api_url):
        op = self.webop
        form_actFAMIC2COrder = self.feature_conf[api_url]
        form_genorder = self.createHtmlFormJs('FormGenOrder', req_info, action=form_actFAMIC2COrder, submit=True)
        print(form_genorder)
    ...
    def modifyAllpaylogisticsSplit(self, modify_str):
        ...
        if type(modify_str) is str:
            ...
            return modify_str[0][20:], modify_str[3][13:]
        else:
            print('####Type_Info####')
