...
class verifyInvQueryAllowance(APIHelper):
    ...
    def verifyInvQueryAllowanceResult(self, req_info, case_id):
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                ...
                raise ValueError("verifyInvQueryAllowanceResult: Length of expected result dic cannot be zero.")
        else:
            raise TypeError("verifyInvQueryAllowanceResult: Received expect_args_dict is not an dictionary")
    ...
    def verifyInvQueryAllowanceRequestInfo(self, req_info):
        ...
        if type(expect_args_dict) is dict:
            ...
            raise TypeError("verifyInvQueryAllowanceRequestInfo: Received expect_args_dict is not an dictionary.")