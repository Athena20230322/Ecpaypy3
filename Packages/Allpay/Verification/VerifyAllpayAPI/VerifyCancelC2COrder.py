...
class verifyCancelC2COrder(APIHelper):
    ...
    def verifyCancelC2COrderCloseResult(self):
        op = self.webop
        ...
        if tag_str.find("TechSupport@allpay.com.tw") != -1:
            result['verifyCancelC2COrderCloseResult'] = True
            return result
        else:
            self.log.WARN("verifyCancelC2COrderCloseResult: result is not find close_message!")
            result['verifyCancelC2COrderCloseResult'] = False
            return result
