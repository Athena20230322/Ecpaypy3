...
class verifyReturnHome(APIHelper):
    ...
    def verifyReturnHomeCloseResult(self):
        op = self.webop
        ...
        if tag_str.find("TechSupport@allpay.com.tw") != -1:
            result['verifyReturnHomeCloseResult'] = True
            return result
        else:
            self.log.WARN("verifyReturnHomeCloseResult: result in not find close_message!")
            result['verifyReturnHomeCloseResult'] = False
            return result