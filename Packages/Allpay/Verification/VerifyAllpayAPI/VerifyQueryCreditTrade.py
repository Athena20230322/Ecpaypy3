...
class verifyQueryCreditTrade(APIHelper):
    ...
    def verifyResponseColumnName(self, res_dict, case_id):
        ...
        self.log.INFO('verifyResponseColumnName\nexpect: %s\nres_dict: %s' % (expect_args_dict, res_dict))
            ...
            if len(expect_args_dict) > 0:
                ...
                for exp in expect_list:
                    if exp in res_dict:
                        ...
                        self.log.WARN('verifyResponseColumnName: column names are different!')
                        result['VerifyReturnKey'] = False
                        return result
    ...
    def verifyQueryInfo(self, res_dict, case_id):
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                ...
                if res_dict['process_date'] != '' and res_dict['process_date'] is not None:
                    ...
                    for i in range(len(res_dict['ExecLog'])):
                        if res_dict['ExecLog'][i]['RtnCode'] != 1:
                            ...
                            self.log.WARN('RtnCode in ExecLog is incorrect!')
                            return result
                    return result
                else:
