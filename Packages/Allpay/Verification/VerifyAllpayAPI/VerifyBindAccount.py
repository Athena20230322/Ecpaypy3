...
class verifyBindAccount(APIHelper):
    def __init__(self):
        self.conf = getConfig()
        self.category = 'AllpayAPI'
        self.fea_name = 'BindAccount'
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                if len(expect_args_dict) != len(res_dict):
                    result['verifyColumnNumbers'] = False
                else:
                    result['verifyColumnNumbers'] = True
                    for key in res_dict:
                        if key in expect_args_dict:
                            result['verifyColumnName'] = True
                            continue
                        else:
                            self.log.WARN('verifyColumnNames different!')
    ...
    def verifyResponseValue(self, res_dict, case_id, request_id):
        all_verify_data = self.t_helper.getTestDataIniSections(self.category, self.fea_name, case_id,
                                                               'Verification.ini')
        ...
        if type(expect_args_dict) is dict:
            if len(expect_args_dict) > 0:
                ...
                if res_dict['AccountID'] == '':
                    result['verifyAccountID'] = False
                return result
            else:
