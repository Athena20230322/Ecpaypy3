...
# from Allpay.Verification.VerifyAllpayAPI.VerifyInvIssueInvalid import verifyInvIssueInvalid
...
from Allpay.ProdActions.ActAllpayAPI.ActInvNotify import actInvNotify
from Allpay.Verification.VerifyAllpayAPI.VerifyInvNotify import verifyInvNotify
...
ISS_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'InvNotify', 'Initial_Data', 'InvIssue.csv')
...
ISSIV_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'InvNotify', 'Initial_Data', 'InvIssueInvalid.csv')

issiv_info = EXEC_ACT(ACT_ISSIV_API.genOrderRequestInfo, ISSIV_INFO_CSV, iss_res_dict['InvoiceNumber'])

issiv_info_res = EXEC_ACT(ACT_ISSIV_API.genPostRequestToAPI, issiv_info)

print 'Issiv_info_res: ', issiv_info_res.decode('utf-8')
...
ALL_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'InvNotify', 'Initial_Data', 'InvAllowance.csv')
...
ALLIV_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'InvNotify', 'Initial_Data', 'InvAllowanceInvalid.csv')
...
VERIFY(VER_ALLIV_API.verifyColumn, alliv_res_dict, 'InvAllowanceInvalid')

'''

# InvNotify

NOTIFY_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'InvNotify', CASE_NAME, 'OrderInfo.csv')

revert_args = ['NotifyMail']

notify_info = EXEC_ACT(ACT_API.genOrderRequestInfo, NOTIFY_INFO_CSV, iss_res_dict, revert_args)
...
VERIFY(VER_API.verifyColumn, notify_res_dict, CASE_NAME)
