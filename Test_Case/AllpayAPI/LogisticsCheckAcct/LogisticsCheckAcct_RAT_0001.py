...
from Allpay.ProdActions.ActAllpayAPI.ActReturnCVS import actReturnCVS
from Allpay.ProdActions.ActAllpayAPI.ActLogisticsCheckAcct import actLogisticsCheckAcct
from Allpay.Verification.VerifyAllpayAPI.VerifyLogisticsCheckAcct import verifyLogisticsCheckAcct
...
ORDER_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'ReturnCVS', 'Initial_Data', 'AioCheckOut.csv')
...
ORDER_INFO_CVS_CRT = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'ReturnCVS', 'Initial_Data', 'Create.csv')
...
ORDER_INFO_C2C = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'ReturnCVS', 'ReturnCVS_RAT_0107', 'OrderInfo.csv')
...
return_cvs = EXEC_ACT(ORDER_API.genpostRequestToAPI, order_info_returncvs,
                      'https://logistics-stage.allpay.com.tw/express/ReturnCVS')
print(return_cvs)

#LogisticsCheckAccoount api
RTN_MERC_TID = EXEC_ACT(ACT_API.getRtnMerTID, return_cvs)

RTN_ORDER_CSV = os.path.join(ROOTDIR, 'Test_Data', 'AllpayAPI', 'LogisticsCheckAcct', CASE_NAME, 'OrderInfo.csv')
...
#VERIFY
VERIFY(VER_API.verifyApiStopServing, response)
