...
INV_ISSUE_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'ECpayAPI', 'InvAllowance', 'Initial_Data', 'InvIssue.csv')
...
invoice_info = EXEC_ACT(ISSUE_API.genPostRequestToAPI, inv_info)

# InvAllowance
INV_ALLOWANCE_INFO_CSV = os.path.join(ROOTDIR, 'Test_Data', 'ECpayAPI', 'InvAllowance', CASE_NAME, 'InvAllowance.csv')

invoice_info_dict = EXEC_ACT(ACT_API.strToDict, invoice_info)

exclusive_list = ['ItemName', 'ItemWord']

revert_list = ['CustomerName', 'NotifyMail', 'ItemName', 'ItemWord']

inv_info = EXEC_ACT(ACT_API.genOrderRequestInfo, INV_ALLOWANCE_INFO_CSV, invoice_info_dict, exclusive_list, revert_list)
