...
CREATE_ORDER_CSV = os.path.join(ROOTDIR, 'Test_Data', 'ECpayAPI', 'Capture', 'Initial_Data', 'CreateOrderInfo.csv')
...
order_info = EXEC_ACT(ACT_API.genCaptureRequestInfo, ORDER_INFO_CSV, '1 or sleep(__TIME__)#')

result_str = EXEC_ACT(ACT_API.captureResult, order_info)

result_dict = ACT_API.strToDict(result_str)

# VERIFY
VERIFY(VER_API.verifyInjectResult, order_info, result_dict, CASE_NAME)
